...
export const metadata: Metadata = {
  ...
  description: 'Interactive map of water quality across all 50 US states.',
};
...
export default async function MapPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <p className="text-gray-600 mb-6">Bubble size = population served. Color = average water quality grade.</p>
        ...
        {['A', 'B', 'C', 'D', 'F'].map(g => (
          <div key={g} className="flex items-center gap-1">
            <div className="w-4 h-4 rounded-full" style={{
              backgroundColor: g === 'A' ? '#22c55e' : g === 'B' ? '#84cc16' : g === 'C' ? '#eab308' : g === 'D' ? '#f97316' : '#ef4444',
            }} />
      ...
      <div className="grid md:grid-cols-2 gap-8 mt-12">
        <div>
          ...
          <div className="space-y-2">
            {worst5.map((s, i) => (
              <Link key={s.state} href={`/search?q=${s.state}`}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-gray-500">{s.systemCount} systems</span>
                </div>
                <div className={`font-bold text-lg ${gradeColorClass(s.grade)}`}>{s.grade} ({s.avgScore})</div>
              </Link>
            ))}
          </div>
        </div>
            ...
            <tbody>
              {stateData.map((s, i) => (
                <tr key={s.state} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="p-3"><Link href={`/search?q=${s.state}`} className="font-semibold hover:text-water-600">{s.state}</Link></td>
                  ...
                  <td className="p-3">{s.fCount > 0 ? <span className="text-red-600 font-semibold">{s.fCount}</span> : <span className="text-green-600">0</span>}</td>
                </tr>
              ))}
            </tbody>
          </table>
