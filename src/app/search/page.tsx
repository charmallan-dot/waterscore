...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    description: q ? `Water quality results for "${q}". Check scores, contaminants, and violations.` : 'Search water quality by zip code or city.',
  };
}
...
export default async function SearchPage({ searchParams }: PageProps) {
  const { q } = await searchParams;
  const query = q || '';
  const results = query ? searchSystems(query, 50) : [];
